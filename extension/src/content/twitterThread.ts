...
export async function autoScrollThread(maxPasses = 6): Promise<number> {
  const scroller =
    document.querySelector('[aria-label^="Timeline"]') || window;
  let lastHeight = 0;
  let passes = 0;
  ...
  for (let i = 0; i < maxPasses; i++) {
    ...
    console.log(
      `[Twitter] Scroll pass ${passes}, height: ${newHeight}`
    );
  }
...
function extractLinks(el: Element): Link[] {
  ...
  for (const a of anchors) {
    ...
    const title =
      (a as HTMLAnchorElement).innerText?.trim() || undefined;
    const expanded =
      (a as HTMLElement).getAttribute("data-expanded-url") || href;
...
function extractMedia(el: Element): Media[] {
  ...
  el.querySelectorAll('img[src*="pbs.twimg.com/media"]').forEach(
    (img) => {
  ...
  el.querySelectorAll("video").forEach((v) => {
    const src =
      (v.querySelector("source[src]") as HTMLSourceElement)?.src ||
...
function getAuthorHandle(el: Element): string {
  ...
  const nameLink = el.querySelector(
    'a[role="link"][href^="/"]'
  ) as HTMLAnchorElement | null;
...
export async function collectTwitterThread(
  ...
  console.log(
    "[Twitter] Starting thread collection with config:",
    config
  );
  ...
  console.log(
    `[Twitter] Collecting thread for ${authorHandle}, root: ${rootId}`
  );
  ...
  for (const art of articles) {
    ...
    if (handle.toLowerCase() !== authorHandle.toLowerCase()) {
      console.log(
        `[Twitter] Skipping tweet from ${handle} (not ${authorHandle})`
      );
      continue;
    }

    const textEl =
      art.querySelector('[data-testid="tweetText"]') || art;
    ...
    console.log(
      `[Twitter] Collected tweet ${tid}: ${text.substring(0, 50)}...`
    );
  }

  const uniq = dedupeBy(tweets, (t) => t.tweet_id).sort((a, b) =>
    (a.created_at || a.tweet_id).localeCompare(
      b.created_at || b.tweet_id
    )
  );
